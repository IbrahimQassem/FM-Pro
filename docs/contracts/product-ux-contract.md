...
Splash/Firebase gate -> Home discovery -> Station details
                                  ...
                                  |                    -> Episode comments
                                  |        -> Weekly schedule
                                  |-> Settings & Account Hub -> ManageAccountScreen / SignInScreen / RegisterScreen (Sub-screens)
...
- لا bottom navigation حاليًا في تطبيق المستمع. لوحة الإدارة المنفصلة هي `web_admin`، ويجري تطوير بديل واجهتها داخل المجلد نفسه. زر الإعدادات يفتح الحساب، وزر
...
- قبل أول تعليق، تعرض شروط المشاركة ويحتاج فتح المحرر إلى موافقة صريحة قابلة
  للتدقيق؛ تبقى الشروط متاحة للمراجعة بعد القبول.
- قائمة إجراءات التعليق توفر بلاغ التعليق وبلاغ المستخدم والحظر كأفعال واضحة.
  يشرح تأكيد الحظر أنه شخصي، وتختفي تعليقات المؤلف فورًا مع تراجع مباشر.
- تعرض لوحة الإدارة طابور البلاغات المفتوحة وسياق التعليق والسبب والتفاصيل، مع
  قرار ذري لرفض البلاغ أو إخفاء/إزالة التعليق أو تعطيل الحساب وحسم البلاغات.

## الحساب والإشعارات

- التصفح والاستماع لا يتطلبان حسابًا، ولا يظهر permission prompt عند التشغيل.
- يدعم الدخول Email/password وGoogle وFacebook، ويظهر Apple على منصات Apple.
  الحساب غير الموثق يرى شاشة رمز واضحة مع إعادة الإرسال والخروج والحذف، ويبقى
  قادرًا على التصفح والاستماع لكنه لا يكتب بيانات شخصية أو UGC.
- تعرض شاشة الحساب طرق الدخول المرتبطة وتسمح بربط طريقة أخرى. تعارض الحساب يوجه
  المستخدم لتسجيل الدخول بالطريقة الحالية ثم الربط، ولا ينشئ UID مكررًا.
- شاشة الحساب توفر حذفًا نهائيًا بعد إعادة المصادقة بكلمة المرور أو المزود
  الاجتماعي وتأكيد صريح، وتوجد صفحة ويب عامة بديلة. لا يتحول الفشل إلى نجاح ولا
  يغلق الحساب محليًا قبل رد الخادم.
