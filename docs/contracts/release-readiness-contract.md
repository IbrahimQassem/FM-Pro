...
- تُراجع قواعد `firestore.rules` وفهارس الجذرين وتختبر عبر Firebase Emulator.
- رفع صور الحساب مؤجل لهذا الإصدار وفق [ADR 0002](../decisions/0002-store-release-boundaries.md)؛ لا يضاف Storage أو تنشر قواعده دون شريحة رفع مستقلة.
...
- لوحة الإدارة (`web_admin`) هي الواجهة المسؤولة عن التحكم في هذه الصفحات العامة وخدمتها.
...
| **بوابة النشر** | استراتيجية الطرح التدريجي | للـAndroid update المؤهل: 5% ومراقبة 24 ساعة على الأقل؛ iOS update وفق جدول المتجر المرحلي. الإطلاق الأول له قرار مستقل. يلزم `Crash-Free Users ≥ 99.5%` مع عينة معتبرة. |

---

## خطة التراجع (Rollback Plan)

1. الاحتفاظ بالمصدر والاعتماديات والرموز والبناء السابق لإعداد إصلاح أمامي برقم إصدار صالح؛ إيقاف الطرح لا يخفض إصدار الأجهزة المحدثة. تراجع خطة المنصة والمصادر في [تقرير الإصدار](../release/store-release-readiness-review-2026-09-06.md).
